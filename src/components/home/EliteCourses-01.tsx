...
interface EliteCourses01Props {
  courses: CourseInfo[];
  loading?: boolean;
  onCourseClick?: (course: CourseInfo) => void;
}

export default function EliteCourses01({ courses, loading, onCourseClick }: EliteCourses01Props) {
  // 获取课程封面图片地址
    ...
    <div className="grid grid-cols-4 gap-4">
      {courses.map((course) => (
        <Card
          key={course.courseId}
          hoverable
          className="cursor-pointer border border-gray-200 transition-all hover:shadow-md overflow-hidden"
          ...
          <div className="flex flex-col">
            {course.coverImage && (
              <img
                ...
                style={{ margin: '-1px -1px 0 -1px', width: 'calc(100% + 2px)' }}
              />
            )}
